...
export const GET = withErrorHandler(async (req: Request) => {
  ...
  // would say and logs it per row beside the legacy pool (to the app stdout →
  // pm2 out log). Never feeds on_hand into computeAvailable, never writes to a
  // marketplace. Best-effort — runLedgerShadow swallows its own errors so it can
  // never disturb the sync.
