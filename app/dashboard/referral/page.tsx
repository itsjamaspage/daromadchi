...
export default async function ReferralPage() {
  const { stats, entries } = await getReferralStats()
      ...
      <div className="flex items-center gap-3">
        <div>
          <div className="flex items-center gap-3 mb-0.5">
            <h1 className="text-2xl font-bold text-white">Referal dasturi</h1>
            <span className="text-[10px] font-semibold px-2 py-1 rounded-full bg-violet-500/10 border border-violet-500/25 text-violet-400">
              {stats.totalReferred} ta do&apos;st
            </span>
          </div>
          <p className="text-slate-400 text-sm">Do&apos;stlaringizni taklif qiling va har bir faol foydalanuvchi uchun mukofot oling</p>
        </div>
