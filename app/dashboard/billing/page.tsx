...
function PlanModal({ current, onClose }: { current: Plan; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-[#13131f] border border-white/[0.08] rounded-2xl shadow-2xl shadow-black/60 overflow-hidden">
        {/* Header */}
        <div className="px-6 py-4 border-b border-white/[0.05] flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Star className="w-4 h-4 text-cyan-400" />
            <h2 className="text-white font-semibold text-sm">Tarifni o&apos;zgartirish</h2>
          </div>
          ...
          {plans.map(plan => {
            ...
            return (
              ...
              >
                ...
                <p className={`font-bold text-base ${isCurrent ? 'text-cyan-300' : 'text-white'}`}>
                  {PLAN_PRICES[plan]}
                </p>
                ...
                {!isCurrent && (
                  <button className="mt-2 w-full bg-violet-600 hover:bg-violet-500 text-white text-xs font-semibold py-2 rounded-lg transition-colors">
                    Tanlash
                  </button>
                )}
                {isCurrent && (
...
function InvoiceModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={onClose} />
      <div className="relative w-full max-w-sm bg-[#13131f] border border-white/[0.08] rounded-2xl shadow-2xl shadow-black/60 overflow-hidden">
        <div className="px-6 py-4 border-b border-white/[0.05] flex items-center justify-between">
          <div className="flex items-center gap-2">
            <FileText className="w-4 h-4 text-violet-400" />
            <h2 className="text-white font-semibold text-sm">Hisob-faktura so&apos;rash</h2>
          </div>
          <button onClick={onClose} className="text-slate-500 hover:text-white transition-colors p-1">
            <X className="w-4 h-4" />
          </button>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-2">Kompaniya nomi</label>
            <input
              type="text"
              value={company}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-2">INN raqami</label>
            <input
              type="text"
              value={inn}
...
export default function BillingPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-white">Tarif va to&apos;lov</h1>
      ...
      {/* ── Current Plan Card ──────────────────────────────────────────────────── */}
      <div className="bg-[#13131f] border border-cyan-500/30 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-white/[0.05] flex items-center gap-2">
          <Zap className="w-4 h-4 text-cyan-400" />
          <h2 className="text-white font-semibold text-sm">Joriy tarif</h2>
        </div>
          ...
          <div className="flex-1 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-slate-500">
                Muddati: <span className="text-slate-300">2026-07-01</span>
              </span>
            </div>
            <p className="text-white font-bold text-xl">249,000 so&apos;m<span className="text-slate-500 font-normal text-sm">/oy</span></p>
      ...
      <div className="bg-[#13131f] border border-white/[0.06] rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-white/[0.05] flex items-center gap-2">
          ...
          <h2 className="text-white font-semibold text-sm">To&apos;lov usullari</h2>
        </div>
        <div className="p-5 space-y-3">
          {/* Saved card */}
          <div className="flex items-center gap-4 bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3">
            ...
            <div className="flex-1">
              <p className="text-white text-sm font-semibold">Uzcard **** 4242</p>
              <p className="text-slate-500 text-xs mt-0.5">Muddati: 12/27</p>
            </div>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            <button className="flex items-center gap-2 text-sm font-medium text-slate-300 hover:text-white border border-white/[0.08] hover:border-white/[0.18] px-4 py-2 rounded-xl transition-all bg-white/[0.02] hover:bg-white/[0.04]">
              <span className="text-violet-400 font-bold text-base leading-none">+</span>
              To&apos;lov usuli qo&apos;shish
            </button>
            <button
              onClick={() => setShowInvoiceModal(true)}
              className="flex items-center gap-2 text-sm font-medium text-slate-300 hover:text-white border border-white/[0.08] hover:border-white/[0.18] px-4 py-2 rounded-xl transition-all bg-white/[0.02] hover:bg-white/[0.04]"
            >
      ...
      <div className="bg-[#13131f] border border-white/[0.06] rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-white/[0.05] flex items-center gap-2">
          <FileText className="w-4 h-4 text-violet-400" />
          <h2 className="text-white font-semibold text-sm">To&apos;lov tarixi</h2>
        </div>
            ...
            <tbody className="divide-y divide-white/[0.03]">
              {MOCK_PAYMENTS.map(row => (
                <tr key={row.id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-white text-xs font-medium">{row.description}</td>
                  ...
                  <td className="px-4 py-3.5 text-center">
                    <button className="text-xs font-medium text-slate-400 hover:text-white border border-white/[0.08] hover:border-white/[0.18] px-3 py-1 rounded-lg transition-all">
                      Chek
                    </button>
                  </td>
                </tr>
