...
const comparisonFeatures: Feature[] = [
  { label: "Do'konlar soni",                     free: '1',      pro: 'Cheksiz', pro_plus: 'Cheksiz' },
  { label: 'Mahsulotlar soni',                   free: '100',    pro: 'Cheksiz', pro_plus: 'Cheksiz' },
  { label: 'Tarix chuqurligi',                   free: '30 kun', pro: '12 oy',   pro_plus: '12 oy'   },
  { label: 'Uzum integratsiya',                  free: true,     pro: true,      pro_plus: true      },
  { label: 'Yandex Market',                      free: false,    pro: true,      pro_plus: true      },
  { label: 'Kengaytma (extension)',              free: true,     pro: true,      pro_plus: true      },
  { label: 'Unit-ekonomika',                     free: false,    pro: true,      pro_plus: true      },
  { label: 'Reklama tahlili',                    free: false,    pro: true,      pro_plus: true      },
  { label: 'Qidiruv iboralari',                  free: false,    pro: true,      pro_plus: true      },
  { label: 'Eksport Excel / PDF',                free: false,    pro: true,      pro_plus: true      },
  { label: 'F&Z (P&L) hisobot',                 free: false,    pro: true,      pro_plus: true      },
  { label: 'Mavsumiylik tahlili',                free: false,    pro: true,      pro_plus: true      },
  { label: 'Narx kuzatuvi',                      free: false,    pro: true,      pro_plus: true      },
  { label: 'Maxsus hisobotlar',                  free: false,    pro: true,      pro_plus: true      },
  { label: 'API kirish',                         free: false,    pro: true,      pro_plus: true      },
  { label: 'Jamoa (5 foydalanuvchi)',             free: false,    pro: false,     pro_plus: true      },
  { label: 'Avtomatik hisobot emailga',           free: false,    pro: false,     pro_plus: true      },
  { label: "Prioritet qo'llab-quvvatlash",        free: false,    pro: false,     pro_plus: true      },
  { label: 'White-label PDF',                    free: false,    pro: false,     pro_plus: true      },
]
...
export default function PricingClient({ prices }: { prices: PricingData }) {
  ...
  const plans = useMemo(() => ([
    ...
    },
    {
      ...
      icon: Shield,
      highlighted: true,
      badge: 'Eng mashhur' as string | null,
      cta: "Sinab ko'ring",
      href: '/login?plan=pro',
      features: PRO_FEATURES as readonly string[],
      extras: [] as typeof PROPLUS_EXTRAS,
    },
    {
      key: 'pro_plus',
  ...
  const faqs: FaqItem[] = useMemo(() => ([
    ...
    },
    {
      ...
      a: `Yillik tarifda 3 oy bepul olasiz — ya'ni 12 oy uchun faqat 9 oylik to'lov. Pro uchun ${somFmt.format(prices.pro.yearly)} so'm/oy, Pro+ uchun ${somFmt.format(prices.pro_plus.yearly)} so'm/oy (yillik to'lov).`,
    },
  ]), [prices])
                ...
                >
                  ...
                  <div className="p-7 flex flex-col flex-1 gap-5">
                    ...
                    <div>
                      ...
                      {currentPrice > 0 && (
                        <p className="text-xs mt-1" style={{ color: plan.highlighted ? 'rgba(255,255,255,0.55)' : 'var(--text-muted)' }}>
                          ≈ ${plan.usd}/oy
                        </p>
                      )}
                      {currentPrice === 0 && (
              ...
              {(['Bepul', 'Pro', 'Pro+'] as const).map((name, idx) => (
                <div key={name} className="p-5 text-center border-l" style={{
                  ...
                  <div className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
                    {idx === 0 ? "0 so'm" : idx === 1 ? `${somFmt.format(prices.pro.monthly)} so'm` : `${somFmt.format(prices.pro_plus.monthly)} so'm`}
                  </div>
                  {idx === 2 && (
