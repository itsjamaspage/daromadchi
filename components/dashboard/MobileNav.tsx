  ...
  return (
    <>
      ...
      <header
        ...
        style={{
          background: theme === 'dark' ? 'var(--bg-card)' : '#0e1b2e',
          borderColor: theme === 'dark' ? 'var(--border)' : 'rgba(255,255,255,0.08)',
        }}
      >
        <button
          onClick={() => setOpen(true)}
          className="p-1 transition-colors"
          style={{ color: theme === 'dark' ? 'var(--text-muted)' : 'rgba(255,255,255,0.7)' }}
          aria-label="Menyuni ochish"
        >
          <Menu className="w-5 h-5" />
        </button>
        <Link href="/dashboard" className="font-bold text-sm tracking-tight flex-1 transition-colors"
          style={{ color: theme === 'dark' ? 'var(--text-base)' : '#ffffff' }}>
          Daromadchi
        </Link>
