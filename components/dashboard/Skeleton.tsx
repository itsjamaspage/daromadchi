  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="bg-[var(--bg-card2)] border border-[var(--border)] rounded-2xl p-5">
...
export function ChartSkeleton({ height = 'h-52' }: { height?: string }) {
  return (
    <div className={`bg-[var(--bg-card2)] border border-[var(--border)] rounded-2xl p-5 ${height}`}>
