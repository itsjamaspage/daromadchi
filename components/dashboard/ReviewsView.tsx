...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1,2,3,4,5].map(i => (
        <Star key={i} className={`w-3 h-3 ${i <= rating ? 'text-amber-400 fill-amber-400' : 'text-[var(--text-dim)]'}`} />
      ))}
    </div>
  )
}
...
function sentimentBg(s: ReviewEntry['sentiment']) {
  return s === 'positive' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'
       ...
       : 'bg-[var(--bg-card2)] border-[var(--border)] text-[var(--text-muted)]'
}
  ...
  return (
    <div className="space-y-4" ref={printRef}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Jami izohlar',   value: String(reviews.length),     color: 'text-[var(--text-base)]'       },
            ...
            <button key={key} onClick={() => setFilter(key)}
              ...
              {count !== undefined && (
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
                  filter === key ? 'bg-violet-500/20 text-violet-300' : 'bg-[var(--bg-input)] text-[var(--text-muted)]'
                }`}>{count}</span>
              )}
            </button>
          ))}
      ...
      <div className="space-y-2">
        {filtered.length === 0 && (
          <div className="bg-[var(--bg-card2)] border border-[var(--border)] rounded-2xl p-10 text-center">
            ...
            <p className="text-[var(--text-muted)] text-sm">Izoh topilmadi</p>
          </div>
        )}
            ...
            <div key={review.id}
              ...
              {isExpanded && review.replyText && (
                <div className="px-4 pb-3 border-t border-[var(--border)] pt-3">
                  <p className="text-xs text-[var(--text-muted)] mb-1.5 font-semibold">Sizning javobingiz:</p>
              ...
              {isReplyOpen && (
                <div className="px-4 pb-4 border-t border-[var(--border)] pt-3 space-y-2">
                  <p className="text-xs text-[var(--text-muted)] font-semibold">Javob yozing:</p>
                  <textarea
                    rows={3}
                    ...
                    className="w-full px-3 py-2 bg-[var(--bg-input)] border border-[var(--border2)] rounded-xl text-sm text-[var(--text-base)] placeholder-[var(--text-muted)] focus:outline-none focus:border-violet-500/50 resize-none"
                  />
