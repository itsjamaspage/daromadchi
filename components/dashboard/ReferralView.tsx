...
function statusLabel(s: ReferralEntry['status']) {
  return s === 'active' ? 'Faol' : s === 'paid' ? 'To\'langan' : 'Kutilmoqda'
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-violet-600/10 to-violet-500/5 border border-violet-500/20 rounded-2xl p-6">
        <p className="text-sm font-semibold text-violet-300 mb-4">Qanday ishlaydi?</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { icon: Copy,       step: '1', text: 'Referal havolangizni nusxalang' },
            { icon: Users,      step: '2', text: 'Do\'stingiz ro\'yxatdan o\'tadi' },
            { icon: Gift,       step: '3', text: 'Har bir faol do\'st uchun 50 000 so\'m mukofot' },
      ...
      <div className="bg-[#13131f] border border-white/[0.06] rounded-2xl p-6 space-y-4">
        <p className="text-xs font-semibold text-slate-400">Sizning referal havolangiz</p>
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 px-4 py-3 bg-[#0d0d1a] border border-white/[0.06] rounded-xl font-mono text-sm text-violet-300 truncate">
            {referralUrl}
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Jami taklif',    value: String(stats.totalReferred),  icon: Users,      color: 'text-white'       },
          ...
          <div key={label} className="bg-[#13131f] border border-white/[0.06] rounded-xl px-4 py-4">
      ...
      {entries.length > 0 && (
        <div className="bg-[#13131f] border border-white/[0.06] rounded-2xl overflow-hidden">
          <div className="px-4 py-3 border-b border-white/[0.05]">
            <p className="text-xs font-semibold text-slate-400">Taklif qilingan foydalanuvchilar</p>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500">Holat</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500">Faollashdi</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500">Mukofot</th>
                </tr>
              </thead>
      ...
      {entries.length === 0 && (
        <div className="bg-[#13131f] border border-dashed border-violet-500/30 rounded-2xl p-10 text-center">
          ...
          <p className="text-white font-semibold mb-1">Hali hech kim taklif qilinmagan</p>
          <p className="text-slate-500 text-sm">Yuqoridagi havolani ulashing va mukofot oling</p>
        </div>
      )}
    </div>
  )
