/**
 * GUARDRAIL: the manual stock reminder can never write to a marketplace.
 *
 ...
 *
 * Run: node --import tsx --test lib/marketplace/manual-stock-notify.guardrail.test.ts
 */
...
// Both halves of the feature. manual-stock-pure.ts is DB-free today, but it is
// where the target number is computed, so it is exactly where a "just push it
// while we're here" change would land.
const ENTRIES = [
  'lib/marketplace/manual-stock-notify.ts',
  'lib/marketplace/manual-stock-pure.ts',
]

describe('guardrail: no marketplace write path is reachable', () => {
  const entry = join(REPO, ENTRIES[0])
  const reachable = new Set([...ENTRIES].flatMap(e => [...reachableFrom(join(REPO, e))]))
  ...
  for (const w of WRITE_CAPABLE) {
    it(`cannot reach ${w}`, () => {
      assert.ok(!reachable.has(join(REPO, w)),
        `manual-stock-notify.ts transitively imports ${w} — a marketplace write is now one call away`)
    })
  }
