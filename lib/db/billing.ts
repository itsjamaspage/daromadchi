...
import { getPendingPriceChange } from '@/lib/billing/price-notice'
...
export interface BillingInfo {
  ...
  /**
   * A price change that has been staged but not charged. Present from the moment
   * it is staged, so the in-app notice appears alongside the Telegram one rather
   * than waiting for delivery to be recorded.
   */
  pendingPrice: { newAmountTiyin: number; effectiveDate: string } | null
}
...
export async function getBilling(): Promise<BillingInfo> {
  const empty: BillingInfo = {
    ...
    cancelledAt: null, accessUntil: null, pendingPrice: null,
  }

  const userId = await getCurrentUserId()
  if (!userId) return empty

  const [[userRow], paymentRows, subRows, cancellation, pendingPrice] = await Promise.all([
    db.select({
      plan: users.plan,
  ...
  return {
    ...
    pendingPrice: pendingPrice && {
      newAmountTiyin: pendingPrice.newAmountTiyin,
      effectiveDate: pendingPrice.effectiveDate.toISOString(),
    },
    derivedTier,
