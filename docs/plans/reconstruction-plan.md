...
> **Cross-marketplace merge (owner request):** ProductsTable now groups by `match_key` (the stock system's proven cross-marketplace grouping key), merging Uzum and Yandex listings that share a normalized SKU or are linked via seller-approved `product_group_merges`. Per-marketplace stock/price shown side by side. Single canonical category per merged row.
...
### Task 14 — 🛑 STOP-REVIEW — Real FBO/FBY stock sync — status: IN PROGRESS (blocked on FBO-enabled account)
...
> **CC note (investigation result — corrected):**
> - **Uzum FBO: EXISTS but untestable with current account.** Uzum supports 4 fulfillment models: FBO (Fulfillment by Operator — seller ships to Uzum warehouse e.g. Sergeli), FBS (seller stores, Uzum delivers), DBS (seller handles everything), EDBS (remote city shipping). The same product card can use FBO+FBS+DBS simultaneously with automatic fallback. The initial investigation found no FBO data because **the test seller account is FBS-only** — it was set up without FBO activation. The API endpoints for FBO likely exist (`/v2/fbo/orders` is already supported in `fetchUzumOrders`, `client.ts:244`) but return no data for FBS-only accounts. A broader probe endpoint (`/api/uzum/fbo-probe`) was built to test all FBO/warehouse/stock paths — needs testing with an FBO-enabled account to confirm which endpoints return data. **Next step:** either activate FBO on the seller account (requires shipping products to Uzum's warehouse) or obtain API access from an FBO-enabled seller.
...
> - **PR #476 contents (safe to merge):** FBY/FBS badge on Settings Yandex card (display-only), fulfillment type query in settings page (display-only), `discoverUzumFboPaths()` in uzum client (read-only diagnostic), FBO paths in diagnose route (read-only), focused FBO probe endpoint (`/api/uzum/fbo-probe`). No write-path code.
