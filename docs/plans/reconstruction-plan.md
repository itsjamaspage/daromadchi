...
### Task 14 — 🛑 STOP-REVIEW — Real FBO/FBY stock sync — status: DONE (partially not feasible)
...
> **CC note (investigation result):**
> - **Uzum FBO: NOT FEASIBLE — no API.** Live probe of Uzum's OpenAPI spec (`/swagger/api-docs`) found zero FBO stock endpoints. Only FBS paths exist: `/v2/fbs/sku/stocks` (GET, POST) and `/v3/fbs/sku/stocks` (GET). All SKUs confirmed `fbsAllowed:true`, `dbsAllowed:false`. FBO appears only as order/invoice labels (`poolSource: FULLFILMENT/SERGELI`), never as queryable stock. Do NOT add an FBO column — it would be permanently blank/misleading.
