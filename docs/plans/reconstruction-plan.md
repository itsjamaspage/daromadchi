...
> **Known gap:** One Yandex M9 watch listing has `category: null` — the Yandex API returns no category in any of the sync fields (offer.category, categoryName, marketCategoryName, mapping.categoryName). This is a marketplace data gap, not a resolver issue. A title-based category inference fallback would fix it but is beyond Task 2 scope.
