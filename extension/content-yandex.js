...
(function () {
  ...
  function calcYm(price, { costPrice=0, packaging=0, adPct=0, volume=1, fby=true, commPct=undefined }={}) {
    ...
    const delivery   = 0;
    // Only real, known costs are shown: marketplace commission + delivery +
    // the seller's own cost price + packaging. The estimated returns, acquiring,
  ...
  function buildYmWidget(attempt=0) {
    ...
    chrome.storage.local.get(['ueSettings','drmLang','drmTheme','drmCommOverride_ym'], data => {
      ...
      commPct=data.drmCommOverride_ym!=null?data.drmCommOverride_ym:getYmCommission();
      render();
    });
    ...
    function render() {
      ...
      ['#drm-ym-cost','#drm-ym-pack','#drm-ym-vol'].forEach(id => { wrap.querySelector(id)?.addEventListener('input',liveRecalc); });
      wrap.querySelector('#drm-ym-comm')?.addEventListener('input',()=>{
        const v=parseFloat(wrap.querySelector('#drm-ym-comm')?.value);
        if(v>0){commPct=v;}
        liveRecalc();
      });
      wrap.querySelector('#drm-ym-comm')?.addEventListener('change',()=>{
        const v=parseFloat(wrap.querySelector('#drm-ym-comm')?.value);
        if(v>0){commPct=v;chrome.storage.local.set({drmCommOverride_ym:v});}
        else{commPct=getYmCommission();chrome.storage.local.remove('drmCommOverride_ym');}
      });
