// Daromadchi — Uzum content script (Yoolip style v3)
(function () {
  'use strict';
  ...
  const LANGS = {
    uz: {
      ...
      narx:'Narx', comm:'Komissiya', delivery:'Yetkazib berish (FBO)',
    ...
    ru: {
      ...
      narx:'Цена', comm:'Комиссия', delivery:'Доставка (FBO)',
    ...
    en: {
      ...
      narx:'Price', comm:'Commission', delivery:'Delivery (FBO)',
  ...
  const COMMISSIONS = {
    kozmetika:12, parfyum:12, gozellik:12, beauty:12,
    elektronika:10, telefon:10, kompyuter:10,
    'oziq-ovqat':8, oziq:8, ovqat:8,
    kiyim:13, ayollar:13, erkaklar:13, bolalar:13,
    uy:14, maishiy:14, mebel:14,
    sport:13, avto:11, default:15,
  };
  ...
  function getCommission() {
    ...
    if (bc) {
      ...
      for (const [k,v] of Object.entries(COMMISSIONS)) if (k!=='default' && t.includes(k)) return v;
    }
  ...
  function buildWidget() {
    ...
    function liveRecalc() {
      ...
      const set = (id,v) => { const e=$('#drm-v-'+id); if(e) e.textContent=v; };
      const col = (id,c) => { const e=$('#drm-v-'+id); if(e) e.style.color=c; };
      ...
      col('profit', color); col('margin', color);
      const bar = $('#drm-profit-bar');
        ...
        <div style="width:360px;max-height:92vh;overflow-y:auto;background:${t.bg};border:1px solid ${t.border};
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;
            ...
            <div style="display:flex;align-items:center;gap:8px">
              ...
              <span style="font-size:10px;font-weight:600;padding:2px 7px;background:#7c3aed;color:#fff;border-radius:20px">Uzum</span>
            </div>
          ...
          <div style="padding:14px 16px;display:flex;flex-direction:column;gap:10px">
            ...
            <div style="display:flex;gap:6px">
              <button id="drm-fbo" style="flex:1;padding:7px;border-radius:8px;border:1px solid ${fbo?'#7c3aed':t.border};background:${fbo?'#7c3aed':'transparent'};color:${fbo?'#fff':t.muted};font-size:12px;font-weight:600;cursor:pointer">${l.fbo}</button>
              <button id="drm-fbs" style="flex:1;padding:7px;border-radius:8px;border:1px solid ${!fbo?'#7c3aed':t.border};background:${!fbo?'#7c3aed':'transparent'};color:${!fbo?'#fff':t.muted};font-size:12px;font-weight:600;cursor:pointer">${l.fbs}</button>
            </div>
            ...
            <!-- ACTIONS -->
            <button id="drm-ue" style="width:100%;padding:11px;background:#16a34a;color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer">
              ${l.ueBtn}
            </button>
            <button id="drm-market" style="width:100%;padding:10px;background:${t.card};color:${t.text};border:1px solid ${t.border};border-radius:10px;font-size:13px;cursor:pointer">
              ${l.marketBtn}
            </button>
  ...
  new MutationObserver(() => {
    if (location.href !== lastUrl) {
      ...
      setTimeout(() => { document.getElementById('drm-widget')?.remove(); init(); }, 1500);
    }
